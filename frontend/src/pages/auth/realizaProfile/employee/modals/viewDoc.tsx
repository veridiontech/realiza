...
export function DocumentViewer({
  ...
  const fetchDocumentByAuditLogId = async (auditLogId: string) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        setOldPdfError(
          `Erro ao carregar a versão do documento: ${err.response?.data?.message || err.message}.`
        );
      } else {
  ...
  ], [traduzirAcao, formatDate, fetchDocumentByAuditLogId]);
  ...
  return (
    <>
      ...
      {showHistory && (
        ...
        >
          <div
            className="relative w-[50rem] h-[90vh] bg-white p-6 rounded-lg shadow-xl flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <button
