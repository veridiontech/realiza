...
export function TableServiceProvider() {
  ...
  const getBranchClient = async () => {
    ...
    try {
      const res = await axios.get(`${ip}/branch/filtered-client?idSearch=${client.idClient}`);
  ...
  const getSupplier = async (idBranch: string) => {
    if (!idBranch) return;
    try {
      const res = await axios.get(`${ip}/supplier/filtered-client?idSearch=${idBranch}`);
    ...
    <div className="border p-10">
      <div className="mb-4">
        <select
          ...
          className="w-[20vw] border rounded-md p-2"
          onChange={(e) => {
      ...
      {suppliers.length > 0 ? (
              ...
              <tr key={supplier.idProvider} className="border border-gray-300">
                ...
                <td className="border border-gray-300 p-2">
                  {supplier.branches && supplier.branches.length > 0 ? (
                    supplier.branches.map((branch: any) => branch.nameBranch).join(", ")
                  ) : (
        ...
        <p className="text-center mt-4">Nenhum fornecedor encontrado.</p>
      )}
    </div>
  );
}
